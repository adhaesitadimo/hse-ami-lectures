...
for file in os.listdir('./'):
    if file == SUBJECT + '_all_lectures.pdf':
        os.remove(os.path.join('./', file))
os.rename('./tex/temp.pdf', SUBJECT + '_all_lectures.pdf')

for elem in os.listdir('./tex/'):
    if os.path.isfile(os.path.join('./', elem)):
        if elem.startswith(SUBJECT) and elem.endswith('.pdf'):
